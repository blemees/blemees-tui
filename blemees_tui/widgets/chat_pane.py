...
class _TurnBlock(Vertical):
    ...
    def render_turn(self, turn: Turn, *, show_thinking: bool = False) -> None:
        ...
        if turn.user_text:
            label = f"[b green]you[/]  > {_escape(turn.user_text)}"
            if self._user is None:
                self._user = Static(label)
    ...
    def _progress_text(self, turn: Turn) -> str:
        ...
        # Token estimate: sum lengths of streamed text blocks divided by 4
        # (rough chars-per-token ratio). Once `agent.result` arrives we use
        # the canonical usage; until then the user gets a live-ish guess.
        chars = 0
...
def _format_tool(block: ToolUseBlock) -> str:
    ...
    head = f"▸ {_escape(block.name)}({_escape(args)})"
    if block.is_error:
        head = f"[red]{head}[/]"
